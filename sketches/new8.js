...
// Vanishing point: horizontally centered, near top of canvas
const VP_X = CANVAS_W / 2;
const VP_Y = CANVAS_H * 0.04;

// The outermost rectangle — wider than canvas, bottom bleeds below canvas
...
// ── Nested rectangles converging to VP at top-center ────────────────────────
...
// ── Bevel effect: diagonal cut lines at bottom corners ───────────────────────
function drawBevel(rx, ry, rw, rh, fr, fg, fb) {
  const bevelSize = max(rw * 0.018, 2);

  // Light edge (bottom and left sides)
  stroke(min(fr + 30, 255), min(fg + 22, 255), min(fb + 18, 255));
  strokeWeight(1.2);
  noFill();

  line(rx + bevelSize, ry + rh, rx + rw - bevelSize, ry + rh);
  line(rx, ry, rx, ry + rh - bevelSize);

  // Dark diagonal corner cuts
  stroke(fr * 0.78, fg * 0.78, fb * 0.78);
  strokeWeight(0.8);
  line(rx,       ry + rh, rx + bevelSize,       ry + rh - bevelSize);
  line(rx + rw,  ry + rh, rx + rw - bevelSize,  ry + rh - bevelSize);

  // Inner bevel line along bottom
  stroke(fr * 0.88, fg * 0.88, fb * 0.88);
  strokeWeight(0.6);
  line(rx + bevelSize, ry + rh - bevelSize, rx + rw - bevelSize, ry + rh - bevelSize);
}
